...
#ifndef MySolarEvent_H
#define MySolarEvent_H 1
...
namespace lowe
{
    class MySolarEvent : public art::EDProducer
    {
    public:
        explicit MySolarEvent(const fhicl::ParameterSet &);
        virtual ~MySolarEvent();
...
namespace lowe
{
    DEFINE_ART_MODULE(MySolarEvent)
}

#endif // MySolarEvent_H
...
namespace lowe
{
    //--------------------------------------------------------------------------
    // Constructor
    MySolarEvent::MySolarEvent(fhicl::ParameterSet const &p)
        : EDProducer{p},
    ...
    //--------------------------------------------------------------------------
    void MySolarEvent::reconfigure(fhicl::ParameterSet const &p)
    {
    ...
    //--------------------------------------------------------------------------
    // Destructor
    MySolarEvent::~MySolarEvent()
    {
        delete producer;
    }
    
    //--------------------------------------------------------------------------
    void MySolarEvent::beginJob()
    {
        // Initialization code if needed
    }
    
    //--------------------------------------------------------------------------
    void MySolarEvent::endJob()
    {
        // Cleanup code if needed
    }
    
    //--------------------------------------------------------------------------
    void MySolarEvent::produce(art::Event &evt)
    {
        std::string sSolarEvent = "MySolarEvent::produce called\n";
        ...
        std::string SolarTruthInfo = "MySolarEvent::produce: MCTruth information for label " + fSignalLabel;
